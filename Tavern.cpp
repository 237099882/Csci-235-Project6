...
void Tavern::createCombatQueue(const std::string& filter)
{
  ...
  {
    std::sort(items_,items_+item_count_,[](Character* x,Character* y)
    {
  ...
  else if (filter=="HPASC")
  {
    std::sort(items_,items_+item_count_,[](Character* x,Character* y)
    {
      return x->getVitality()<y->getVitality();
    }
    );
    for(int i = 0;i<item_count_;i++)
    {   
...
void Tavern::printCombatQueue()
{
  ...
  {
    Character* temp = combat_queue_.front();
    std::cout<<"(ENEMY)"<<temp->getName()<<": LEVEL "<<temp->getLevel()<<" "<<temp->getRace()<<". \nVITALITY: "<<temp->getVitality()<<" \nARMOR: "<<temp->getArmor()<<"\n";
    combat_queue_.pop();
    combat_queue_.push(temp);
  }
}
/** 
...
void Tavern::actionSelection()
{
  ...
  for(int i=0;i<main_character_->getAction().size();i++)
  {
    main_character_->removeAction();
  }
  std::cout<<"YOUR TURN\n";
  std::cout<<"(ENEMY) ";
  Character* temp = combat_queue_.front();
  std::cout<<temp->getName()<<": LEVEL "<<temp->getLevel()<<" "<<temp->getRace()<<". \nVITALITY: "<<temp->getVitality()<<" \nARMOR: "<<temp->getArmor()<<"\n\n";
  for(int j = 0;j<3;j++)
  {
...
void Tavern::turnResolution()
{
  ...
  while(!main_character_->getAction().empty())
  {
    ...
    if(action == 1)
    {
      Buff heal = {"BUFF_Heal",3};
      main_character_->heal();
      heal.turns_--;
      main_character_->setBuff(heal);
      action_name="Heal";
    }
    else if (action == 2)
    {
      Buff mend = {"BUFF_MendMetal",2};
      main_character_->mendMetal();
      mend.turns_--;
      main_character_->setBuff(mend);
      action_name="MendMetal";
    }
    else if (action == 3)
    {
    ...
    std::cout<<"\n(ENEMY) "<<target->getName()<<": LEVEL "<<target->getLevel()<<" "<<target->getRace()<<". \nVITALITY: "<<target->getVitality()<<" \nARMOR: "<<target->getArmor()<<"\n";
...
void Tavern::enemyTurn(Character* enemy)
{
  ...
  int enemy_action = rand()%4+1;
    ...
    {
      main_character_->throwTomato(main_character_);
      action_name="ThrowTomato";
    }
    std::cout<<enemy->getName()<<" used "<<action_name<<"!";
...
//helper function to ask input for action
void Tavern::inputAction()
{
    int a;
    while (true)
    {
