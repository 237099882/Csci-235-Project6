...
//helper function to access action and buffs 
std::queue<int> Character::getAction() const
{
    return action_queue_;
}

void Character::setAction(int a)
{
    action_queue_.push(a);
}

void Character::removeAction()
{
    action_queue_.pop();
}

std::stack<Buff> Character::getBuff() const
{
    return buff_stack_;
}

void Character::setBuff(Buff buff)
{
...
{
    std::stack<Buff> temp_stack;
    Buff temp;
    for(int i = 0;i<buff_stack_.size();i++)
    {
        temp = buff_stack_.top();
        if(temp.name_=="BUFF_Heal")
        {
            heal();
        }
        if(temp.name_=="BUFF_MendMetal")
        {
            mendMetal();
        }
        temp.turns_--;
